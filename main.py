...
with st.sidebar:
    ...
    with st.expander("역할 지침 수정하기", expanded=False):
        st.caption("현재 역할 지침:")
        st.code(st.session_state.role_instruction, language="text")
    ...
    with st.expander("작업 지침 수정하기", expanded=False):
        ...
        st.code(st.session_state.task_instruction, language="text")
...
with input_col:
    ...
    # class를 직접 달 수 없어서, 바로 위 CSS는 전체 input을 바꾸지만
    # 필요한 경우 나중에 더 세밀하게 조정 가능.
